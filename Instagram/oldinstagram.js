// username.json tag numberofelements  true/undefined
...
(async function main() {
    try {
        ...
        if (process.argv[5] == "true") {
            let exist = await fs.access(`history/${process.argv[3]}.json`, err => err ? false : true);
            if (exist) {
                ...
                console.log("nah man")
                let data = { last: addedPost[addedPost.length - 1].id, content: addedPost }
